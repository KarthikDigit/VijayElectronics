...
import com.vijayelectronics.databases.User_Recents_DB;
import com.vijayelectronics.fragments.Checkout;
import com.vijayelectronics.fragments.My_Cart;
import com.vijayelectronics.fragments.Product_Description;
...
import com.vijayelectronics.models.product_model.ProductMetaData;
...
public class CartItemsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holderAll, int position) {
        ...
        if (LAYOUT_ONE == 0) {
            ...
            holder.cart_item_base_price.setText(ConstantValues.CURRENCY_SYMBOL + new DecimalFormat("#0.00").format(Double.parseDouble(TextUtils.isEmpty(cartDetails.getPrice()) ? "0" : cartDetails.getPrice())));
            holder.cart_item_sub_price.setText(ConstantValues.CURRENCY_SYMBOL + new DecimalFormat("#0.00").format(Double.parseDouble(TextUtils.isEmpty(cartDetails.getPrice()) ? "0" : cartDetails.getPrice()) * Double.parseDouble(product_qty)));
